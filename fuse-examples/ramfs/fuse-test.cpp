...
/** Converts a string to a map with individual bytes */
...
/** Removes a potentially existing / at the beginning */
...
static int ramfs_getattr(const char* path, struct stat* stbuf) {
  ...
  if(filename == "/") { // Attributes of the root directory
    cout << "ramfs_getattr("<<filename<<"): Returning attributes for /" << endl;
    stbuf->st_mode = S_IFDIR | 0777;
    stbuf->st_nlink = 2;

  } else if(file_exists(stripped_slash)) { // An existing file is read
...
static int ramfs_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi) {
  ...
  // File system has no subdirectories
  if(strcmp(path, "/") != 0)  {
    cout << "ramfs_readdir("<<path<<"): Only / allowed" << endl;
    return -ENOENT;
  }

  // These files must always exist
  filler(buf, ".", NULL, 0);
  filler(buf, "..", NULL, 0);
...
static int ramfs_read(const char* path, char* buf, size_t size, off_t offset,
  ...
  // True if file exists. Reading in buffer.
  FileContents &file = files[filename];
  size_t len = file.size();

  // Check how many bytes can be read at what offset
  if (offset < len) {
    if (offset + size > len) {
  ...
  } else { // Offset was larger than the maximum size of the file
    return -EINVAL;
  }
...
int ramfs_write(const char* path, const char* data, size_t size, off_t offset, struct fuse_file_info*) {
  ...
  // True if file exists. Write in buffer
...
int ramfs_create(const char* path, mode_t mode, struct fuse_file_info *) {
  ...
  // It tries to create something other than a regular file.
  if( (mode & S_IFREG) == 0)  {
...
// Contains the function pointer to the implemented operations
static struct fuse_operations ramfs_oper;

int main(int argc, char** argv) {
  // Assigning the individual function pointer
